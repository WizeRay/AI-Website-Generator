...
function Community() {
  ...
  useEffect(()=>{
    fetchProjects()
  },[]);//as it should only run fetch once, else infinite loop will occur
            ...
            <div className="flex flex-wrap gap-3.5">
              {projects.map((project)=>(
                 ...
                 className=" w-72 max-sm:mx-auto cursor-pointer bg-gray-900/60 border border-gray-700 rounded-lg overflow-hidden  group hover:shadow-indigo-700/30 hover:border-indigo-800/80 transition-all duration-300">
                  ...
                  {/*Content */}
                  <div className="p-4 text-white bg-gradient-180 from-transparent group-hover:from-indigo-950 to-transparent transition-colors">
                    <div className=" flex items-start justify-between">
                      <h2 
                      className ="text-lg font-medium line-clamp-2">{project.name}</h2>
                      <button 
                      className="px-2.5 py-0.5 mt-1 ml-2 text-xs bg-gray-800 border border-gray-700 rounded-full">Website
                      </button>
                    </div>
